import { Expense, CategoryStats, IncomeEntry } from '../types';
import { getCategoryById } from '../../features/expenses/constants/categories';
...
export const searchExpenses = (expenses: Expense[], query: string): Expense[] => {
  ...
  return expenses.filter((expense) => expense.description.toLowerCase().includes(lowercaseQuery));
};
...
export const getRecentExpenses = (expenses: Expense[], count = 5): Expense[] => {
