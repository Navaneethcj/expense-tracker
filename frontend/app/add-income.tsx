...
import { Header, InputField, PrimaryButton } from '@frontend/shared/components';
import { Colors, Typography, Spacing, BorderRadius } from '@frontend/theme';
import { IncomeEntry } from '@frontend/shared/types';
import { createIncome, getIncomes, updateIncome } from '@frontend/features/income/services/income';
...
export default function AddIncomeScreen() {
  ...
  useEffect(() => {
    ...
    const loadIncome = async () => {
      ...
      const existingIncome = incomes.find((item: any) => item.id === editId);
      if (existingIncome) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditing) {
        await updateIncome((Array.isArray(params.editId) ? params.editId[0] : params.editId) || generateId(), incomeData);
      } else {
