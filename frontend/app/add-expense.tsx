...
import { Header, InputField, CategoryChip, PrimaryButton } from '@frontend/shared/components';
...
import { createExpense, getExpenses, updateExpense } from '@frontend/features/expenses/services/expense';
import { generateId } from '@frontend/shared/utils';
...
export default function AddExpenseScreen() {
  ...
  useEffect(() => {
    ...
    const loadExpense = async () => {
      ...
      const existingExpense = expenses.find((item: any) => item.id === editId);
      if (existingExpense) {
        setAmount(existingExpense.amount.toString());
        setSelectedCategory(existingExpense.category);
        setDescription(existingExpense.description || '');
        setDate(existingExpense.date?.split('T')[0] || new Date().toISOString().split('T')[0]);
      }
    };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditing) {
        await updateExpense((Array.isArray(params.editId) ? params.editId[0] : params.editId) || generateId(), expenseData);
      } else {
