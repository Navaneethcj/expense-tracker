...
import { Expense, IncomeEntry, CategoryStats } from '@frontend/shared/types';
import { deleteExpense, getExpenses } from '@frontend/features/expenses/services/expense';
import { getHistory, getIncomes } from '@frontend/features/income/services/income';
import { formatCurrency, getMonthlyHistory, formatMonthYear, getMonthlyTotal, getMonthlyIncomes, getHighestCategory, filterExpensesByMonth, filterIncomesByMonth, MonthlySummary } from '@frontend/shared/utils';
import { getCategoryById } from '@frontend/features/expenses/constants/categories';
import { ExpenseCard } from '@frontend/shared/components';
...
export default function MonthlyHistoryScreen() {
  ...
  const loadData = async () => {
    try {
      const [expenseData, incomeData, historyData] = await Promise.all([
        getExpenses(),
        getIncomes(),
        getHistory(),
      ]);
      ...
      const history = historyData.map((item: any) => ({
        month: item.month,
        monthName: new Date(`${item.month}-01`).toLocaleString('default', { month: 'long', year: 'numeric' }),
        totalIncome: item.income,
        totalExpenses: item.expense,
        savings: item.balance,
